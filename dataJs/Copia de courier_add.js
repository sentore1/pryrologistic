...
function calculateFinalTotal(element = null) {
  ...
  var use_auto_calc = $("#use_auto_calc").is(":checked");
  ...
  if (use_auto_calc) {
    sumador_total = sumador_libras * price_lb; // Calculate total based on weight and price per lb
  } else {
...
document.addEventListener("DOMContentLoaded", function () {
  var useAutoCalcCheckbox = document.getElementById("use_auto_calc");
  var calculateInvoiceButton = document.getElementById("calculate_invoice");

  // Función para activar o desactivar el botón calculate_invoice
  function toggleCalculateInvoiceButton() {
    if (useAutoCalcCheckbox.checked) {
      calculateInvoiceButton.disabled = true;
    } else {
      calculateInvoiceButton.disabled = false;
    }
  }

  // Escuchar cambios en la casilla de verificación
  useAutoCalcCheckbox.addEventListener("change", function () {
    toggleCalculateInvoiceButton();
  });
