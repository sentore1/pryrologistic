	...
	class TCPDF {
		...
		protected function setFooter() {
		    ...
		    if ($this->print_footer) {
		        ...
		        // Check if footer_font is properly defined
		        if (isset($this->footer_font[0]) && isset($this->footer_font[1]) && isset($this->footer_font[2])) {
		            $this->SetFont($this->footer_font[0], $this->footer_font[1], $this->footer_font[2]);
		        } else {
		            // Set a default font if footer_font is not properly defined
		            $this->SetFont('helvetica', '', 10); // Usa los valores que creas apropiados
		        }
		...
		public function AddFont($family, $style='', $fontfile='') {
		    ...
		    // This line is modified to handle possible null values
		...
		protected function ImagePngAlpha($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='') {
		    ...
		    for ($xpx = 0; $xpx < $wpx; ++$xpx) {
		        for ($ypx = 0; $ypx < $hpx; ++$ypx) {
		            ...
		            $alpha = $this->getGDgamma($img, $colorindex);
		...
		protected function getGDgamma($img, $c) {
		    if (!isset($this->gdgammacache['#'.$c])) {
		        ...
		        $alpha = (127 - $colors['alpha']) * 2;
		        $gamma_corrected = pow(($alpha / 255), 2.2) * 255;
		        $this->gdgammacache['#'.$c] = (int)$gamma_corrected;
		        
		        // store the latest values on cache to improve performance
